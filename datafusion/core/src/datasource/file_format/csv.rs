...
"#,
    ...
    /// Test that CsvSink exposes rows_written, bytes_written, and
    /// elapsed_compute metrics via DataSinkExec.
    #[tokio::test]
    async fn test_csv_sink_metrics() -> Result<()> {
        use datafusion_execution::TaskContext;
        use futures::TryStreamExt;

        let ctx = SessionContext::new();
        let tmp_dir = tempfile::TempDir::new()?;
        let output_path = tmp_dir.path().join("metrics_test.csv");
        ...
        let df = ctx
            .sql(&format!(
                "COPY source TO '{output_path_str}' STORED AS CSV"
            ))
            .await?;
        ...
        let metrics = plan
            .metrics()
            .expect("DataSinkExec should return metrics from CsvSink");
